...
function getRestingTeams(matches: Match[], round: 'round1' | 'round2' | 'round3' | 'round4'): string[] {
...
export function HomeView() {
  ...
  const round4 = matches.filter(m => m.round === 'round4');
  ...
  const restingRound4 = getRestingTeams(matches, 'round4');
  ...
  return (
    <div className="p-4 pb-24">
      ...
      <AccordionItem title="Round 3 (5:06 PM)">
      ...
      <AccordionItem title="Round 4 (5:19 PM)">
        {restingRound4.length > 0 && <RestingTeamsBadge teams={restingRound4} teamNames={teamNames} />}
        <div className="space-y-3">
          {round4.map(match => (
            <MatchCard
              key={match.id}
